...
Mutex KineticNotifier::initMutex;
std::map<std::string, KineticNotifier *> KineticNotifier::instances;
uint16_t KineticNotifier::refCount = 0;
/*
 * Implementation of the member functions in the KineticNotifier class
 */
KineticNotifier::KineticNotifier(EPStats &st, Configuration &config) :
...
void KineticNotifier::resetConnection() {
...
void KineticNotifier::handleResponse(protocol_binary_response_header *res) {
    std::list<BinaryPacketHandler*>::iterator iter;
...
void KineticNotifier::ensureConnection()
{
    if (!connected) {
        // I need to connect!!!
        std::stringstream rv;
...
bool KineticNotifier::waitForWritable()
{
    size_t timeout = 1000;
    size_t waitTime = 0;
...
void KineticNotifier::sendSingleChunk(const char *ptr, size_t nb)
{
    while (nb > 0) {
...
void KineticNotifier::sendCommand(BinaryPacketHandler *rh)
{
...
void KineticNotifier::maybeProcessInput()
{
    struct pollfd fds;
    fds.fd = sock;
    fds.events = POLLIN;
...
bool KineticNotifier::processInput() {
...
bool KineticNotifier::waitForReadable(bool tryOnce)
{
    size_t timeout = 1000;
    size_t waitTime = 0;
...
void KineticNotifier::delVBucket(uint16_t vb, Callback<bool> &cb) {
...
void KineticNotifier::flush(Callback<bool> &cb) {
...
void KineticNotifier::selectBucket() {
...
void KineticNotifier::notify_update(const VBStateNotification &vbs,
...
void KineticNotifier::addStats(const std::string &prefix,
                             ADD_STAT add_stat,
                             const void *c)
{
...
const char *KineticNotifier::cmd2str(uint8_t cmd)
{
    switch(cmd) {
...
int KineticNotifier::commandId(uint8_t opcode) {
    switch(opcode) {
    case PROTOCOL_BINARY_CMD_DEL_VBUCKET:
...
const char *KineticNotifier::cmdId2str(int id) {
    switch(id) {
